package com.ChessFormer;

public class Game_Utilz {

    public static final int TILE_SIZE = 60; // Kích thước ô cờ
    public static float UNIT_SCALE = 1/32f; // Tỉ lệ đơn vị
    public static float UNIT_SIZE  = 32f;

    public static final String MAP_FOLDER_PATH = "Map_Assets/"; // Đường dẫn đến thư mục chứa bản đồ

    // Kích thước màn hình game - ngang 20 ô, dọc 12 ô
    public static final int VIEWPORT_WIDTH = 20;
    public static final int VIEWPORT_HEIGHT = 12;

    // Số lượng Map tối đa
    public static final int MAP_LEVEL_MAX = 15;

    public static final float GRAVITY = 9.8f;

}
